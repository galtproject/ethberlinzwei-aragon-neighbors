...
//    constructor(DAOFactory _daoFactory, ENS _ens, MiniMeTokenFactory _miniMeTokenFactory, address _aragonId) TemplateBase(_daoFactory, _ens) public {
    constructor(DAOFactory _daoFactory, ENS _ens, MiniMeTokenFactory _miniMeTokenFactory) TemplateBase(_daoFactory, _ens) public {
      tokenFactory = _miniMeTokenFactory;
    }

    function getMyHash() external view returns (bytes32) {
...
//        bytes32 appId = keccak256(abi.encodePacked(apmNamehash("open"), keccak256("neighbors")));
        bytes32 appId = apmNamehash("counter-app");
